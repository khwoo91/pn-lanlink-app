...
export class LlHeader extends LitElement {
        ...
        <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div @click=${this.onLogoClick} class="flex items-center space-x-3 cursor-pointer select-none">
            ...
            <div class="flex items-baseline space-x-2">
              ...
              <span class="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-500 border border-slate-200 dark:border-slate-700 text-[9px] font-bold">v1.1.3</span>
          ...
          <div class="flex items-center space-x-2 md:space-x-3">
            <!-- Nickname -->
            <div class="flex items-center space-x-1 bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 text-xs transition-colors">
            ...
            <div class="bg-slate-100 dark:bg-slate-800 p-1 rounded-lg flex items-center border border-slate-200 dark:border-slate-700">
              <button @click=${() => this.onThemeChange('light')} class="p-1.5 rounded-md transition ${this.currentTheme === 'light' ? 'bg-white dark:bg-slate-950 text-google-blue shadow-sm' : 'text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-200'}" title="밝은 테마">
                <i data-lucide="sun" class="w-3.5 h-3.5"></i>
              </button>
              <button @click=${() => this.onThemeChange('dark')} class="p-1.5 rounded-md transition ${this.currentTheme === 'dark' ? 'bg-white dark:bg-slate-950 text-google-blue shadow-sm' : 'text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-200'}" title="어두운 테마">
                <i data-lucide="moon" class="w-3.5 h-3.5"></i>
              </button>
              <button @click=${() => this.onThemeChange('system')} class="p-1.5 rounded-md transition ${this.currentTheme === 'system' ? 'bg-white dark:bg-slate-950 text-google-blue shadow-sm' : 'text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-200'}" title="시스템 테마 연동">
                <i data-lucide="monitor" class="w-3.5 h-3.5"></i>
              </button>
            </div>

            <!-- LAN state -->
            <div class="hidden md:flex items-center space-x-1.5 bg-slate-100 dark:bg-slate-800/60 px-2.5 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 transition-colors text-[11px]">
  ...
  private onThemeChange(theme: string) {
    this.dispatchEvent(new CustomEvent('change-theme', { detail: { theme }, bubbles: true, composed: true }));
  }

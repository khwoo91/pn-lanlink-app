      ...
      <header class="border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/90 backdrop-blur-md sticky top-0 z-40 transition-colors duration-200">
        <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div class="flex items-center space-x-3">
            <div class="w-9 h-9 rounded-lg bg-[#1a73e8] flex items-center justify-center shadow-md">
          ...
          <div class="flex items-center space-x-2 md:space-x-3">
            ...
            <div class="flex items-center space-x-1 bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-700 text-xs transition-colors">
              <div class="w-4 h-4 rounded-full bg-[#1a73e8]/10 text-[#1a73e8] flex items-center justify-center">
                <i data-lucide="user" class="w-3.5 h-3.5"></i>
              </div>
              <span class="text-slate-500 dark:text-slate-400 hidden sm:inline">내 닉네임:</span>
              <strong class="text-slate-800 dark:text-slate-200 font-bold max-w-[80px] truncate">${this.currentNickname}</strong>
              <button @click=${this.onEditNickname} class="text-slate-400 hover:text-[#1a73e8] p-0.5 rounded transition" title="닉네임 수정">

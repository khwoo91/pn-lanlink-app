      ...
      <div class="border-t border-slate-200 dark:border-slate-800 pt-6 space-y-4">
        ...
        <div class="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 font-semibold px-1">
          <span class="flex items-center gap-1.5">
            <i data-lucide="search" class="w-3.5 h-3.5 text-goggle-blue"></i> 사내 네트워크 대기방 감지
          </span>
          <span class="text-[10px] text-goggle-blue animate-pulse">실시간 탐색 중...</span>
        </div>

        <!-- Scanned list -->
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ${this.rooms.map(room => html`
            <div class="p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl hover:border-goggle-blue transition group flex items-center justify-between text-xs">
              <div class="space-y-1">
                <div class="flex items-center gap-1.5">
                  <span class="font-bold text-slate-800 dark:text-slate-200">${room.name}</span>
                  ${room.locked ? html`
                    <span class="bg-goggle-blue/10 text-goggle-blue text-[9px] px-1.5 py-0.5 rounded font-semibold flex items-center gap-0.5">
              ...
              <button @click=${() => this.onSelectRoom(room)} class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-goggle-blue hover:text-white dark:hover:bg-goggle-blue px-2.5 py-1.5 rounded-md font-semibold transition text-slate-800 dark:text-slate-200">
                참여
              </button>
            </div>
          `)}

      ...
      <div id="landing-visual-panel" class="flex flex-col items-center justify-center text-center">
        <div
          ...
          <div class="flex items-center justify-center space-x-1.5 pt-2">
            ${Array.from({ length: this.carouselData.length }).map((_, index) => html`
              <button @click=${() => this.onSwitchCarousel(index)} id="carousel-dot-${index}"
                class="${this.carouselIndex === index ? 'w-2.5 h-2.5 bg-google-blue' : 'w-2 h-2 bg-slate-300 dark:bg-slate-700'} rounded-full transition-all"></button>
            `)}
          </div>
        </div>
      </div>

  ...
  render() {
    return html`
      <div id="landing-visual-panel" class="flex flex-col items-center justify-center text-center">
        <div
          class="custom-shadow w-full max-w-md space-y-6 rounded-2xl border border-slate-200 bg-slate-50 p-8 dark:border-slate-800 dark:bg-slate-900"
        >
          <div
            class="relative mx-auto flex h-48 w-48 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800"
          >
            <svg
              class="text-google-blue h-32 w-32"
              viewBox="0 0 100 100"
              fill="none"
              stroke="currentColor"
              stroke-width="2"
            ...
            >
              ...
              <i data-lucide="link" class="absolute top-22.5 left-25 h-4 w-4 text-white"></i>
            </svg>
          </div>

          <div class="space-y-2">
            <h2 id="carousel-title" class="text-base font-extrabold text-slate-900 dark:text-white">
              ${this.carouselData[this.carouselIndex].title}
            </h2>
            <p id="carousel-desc" class="mx-auto max-w-xs text-xs leading-relaxed text-slate-500 dark:text-slate-400">
          ...
          <div class="flex items-center justify-center space-x-1.5 pt-2">
            ${Array.from({ length: this.carouselData.length }).map(
              (_, index) => html`
                <button
                  @click=${() => this.onSwitchCarousel(index)}
                  id="carousel-dot-${index}"
                  class="${this.carouselIndex === index
                    ? "w-2.5 h-2.5 bg-google-blue"
                    : "w-2 h-2 bg-slate-300 dark:bg-slate-700"} rounded-full transition-all"
                ></button>
              `
            )}
          </div>

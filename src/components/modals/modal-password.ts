    ...
    return html`
      <div id="password-verify-modal" class="${this.open ? '' : 'hidden'} fixed inset-0 z-50 bg-slate-950/75 backdrop-blur-md flex items-center justify-center p-4">
        <div class="bg-white dark:bg-slate-900 border-2 border-indigo-500/30 rounded-2xl max-w-sm w-full p-6 text-center shadow-2xl">
          <div class="w-12 h-12 bg-indigo-500/10 border border-indigo-500/20 text-[#1a73e8] rounded-full flex items-center justify-center mx-auto mb-3">
          ...
          <div class="my-4">
            <input type="password" id="input-verify-password" placeholder="비밀번호 4자리 입력" class="w-full text-center py-2.5 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:border-[#1a73e8] focus:outline-none font-mono text-sm tracking-widest text-slate-800 dark:text-white">
          </div>
          ...
          <div class="flex gap-2">
            ...
            <button @click=${this.onSubmit} class="flex-grow bg-[#1a73e8] hover:bg-[#1557b0] text-white font-bold py-2.5 rounded-xl text-xs transition">
              인증 및 접속
            </button>
          </div>
        </div>

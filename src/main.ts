  ...
  render() {
    return html`
      ...
      <main class="grow max-w-7xl mx-auto px-6 py-12 md:py-20 w-full flex flex-col justify-center">
        ...
        <div id="sharing-active-workspace"
          class="max-w-3xl mx-auto w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 space-y-8 custom-shadow animate-in zoom-in-95 duration-300">
        ...
        <ll-viewer .activeRoomName=${this.activeRoomName} .activeRoomIp=${this.activeRoomIp} .localMuted=${this.localMuted}
          ...
          .cursorY=${this.cursorY} @toggle-mute=${this.toggleLocalMute} @simulate-click=${this.simulateClick}
          @toggle-draw=${this.toggleAnnotationOverlay} @leave-session=${this.leaveSession}>
          <!-- Slot children in viewer grid layout -->
          <ll-chat .chatMessages=${this.chatMessages} .viewerCount=${this.viewerCount} @send-message=${this.onSendMessage}>
          </ll-chat>
